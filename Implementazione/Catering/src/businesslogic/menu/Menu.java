...
public class Menu {
    ...
    private ObservableList<MenuItem> freeItems = FXCollections.observableArrayList();
    ...
    public String toString() {
        return title + " (autore: " + owner.getId() + ")," + (published ? " " : " non ") +
    ...
    public ObservableList<KitchenTask> getRecipes(){
        ObservableList<KitchenTask> kitchenTasks = FXCollections.observableArrayList();
        for (Section s: this.sections) {
            for (MenuItem item : s.getSectionItems()) {
                kitchenTasks.add(item.getItemRecipe());
                kitchenTasks.addAll(item.getItemRecipe().getIngredients());
            }
        }
    ...
    public static ObservableList<Menu> loadAllMenus() {
        ...
        for (int i = 0; i < newMenus.size(); i++) {
            ...
            m.owner = UserManager.getOrganizerById(newMids.get(i)); //User.loadUserById(newMids.get(i));
        ...
        for (int i = 0; i < oldMenus.size(); i++) {
            ...
            m.owner = UserManager.getOrganizerById(oldMids.get(i)); //User.loadUserById(oldMids.get(i));
    ...
    public static Menu loadMenuById(int id) {
        ...
        PersistenceManager.executeQuery(query, new ResultHandler() {
            ...
            public void handle(ResultSet rs) throws SQLException {
                ...
                menu.owner = UserManager.getOrganizerById(rs.getInt("owner_id"));
                menu.inUse = rs.getBoolean("inUse");
                menu.published = rs.getBoolean("published");
            }
        });

        menu.sections.addAll(Section.loadSectionsFor(id));
        for(Section section: menu.sections){
            menu.freeItems.addAll(MenuItem.loadItemsFor(id, 0));
        }

        if(menu.id > 0 && !loadedMenus.containsKey(menu.id)) {
