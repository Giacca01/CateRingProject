...
public class User {
    ...
    public static User loadUser(String username) {
        ...
        if (u.id > 0) {
            ...
            String eventsQuery = "SELECT * FROM Events WHERE organizer_id=" + u.id;
            PersistenceManager.executeQuery(eventsQuery, new ResultHandler() {
                @Override
                public void handle(ResultSet rs) throws SQLException {
                    u.assignedEvents.add(EventManager.getEventById(rs.getInt("id")));
                }
            });
