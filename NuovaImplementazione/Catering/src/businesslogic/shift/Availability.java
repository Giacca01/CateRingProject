...
public class Availability {
    ...
    /*public static ObservableList<Availability> fetchAvailabilities() {
        ...
        ArrayList<Availability> newAvailabilities = new ArrayList<>();
        ArrayList<Integer> newEmployees = new ArrayList<>();
        ArrayList<Integer> newServices = new ArrayList<>();
        ArrayList<Integer> newShifts = new ArrayList<>();
        ArrayList<Integer> newReservedBys = new ArrayList<>();
        ArrayList<Availability> oldAvailabilities = new ArrayList<>();
        ArrayList<Integer> oldEmployees = new ArrayList<>();
        ArrayList<Integer> oldServices = new ArrayList<>();
        ArrayList<Integer> oldShifts = new ArrayList<>();
        ArrayList<Integer> oldReservedBys = new ArrayList<>();
        ...
        PersistenceManager.executeQuery(query, new ResultHandler() {
            ...
            public void handle(ResultSet rs) throws SQLException {
                ...
                if(loadedAvailabilities.containsKey(id)) {
                    ...
                    a.reserved = rs.getBoolean("reserved");
                    a.role = rs.getString("role");
                    oldEmployees.add(rs.getInt("employee_id"));
                    oldServices.add(rs.getInt("service_id"));
                    oldShifts.add(rs.getInt("shift_id"));
                    oldReservedBys.add(rs.getInt("reserved_by"));
                    ...
                    a.reserved = rs.getBoolean("reserved");
                    a.role = rs.getString("role");
                    newEmployees.add(rs.getInt("employee_id"));
                    newServices.add(rs.getInt("service_id"));
                    newShifts.add(rs.getInt("shift_id"));
                    newReservedBys.add(rs.getInt("reserved_by"));
        ...
        for (int i = 0; i < newAvailabilities.size(); i++) {
            Availability a = newAvailabilities.get(i);
            if(newEmployees.get(i) != 0) a.employee = Cook.loadCookById(newEmployees.get(i));
            if(newServices.get(i) != 0) a.service = Service.loadServiceById(newServices.get(i));
            if(newShifts.get(i) != 0) a.shift = Shift.loadShiftById(newShifts.get(i));
            if(newReservedBys.get(i) != 0) a.reservedBy = UserManager.getOrganizerById(newReservedBys.get(i));
            //a.reservedBy = null;
        }

        for (int i = 0; i < oldAvailabilities.size(); i++) {
            Availability a = oldAvailabilities.get(i);
            if(oldEmployees.get(i) != 0) a.employee = Cook.loadCookById(oldEmployees.get(i));
            if(oldServices.get(i) != 0) a.service = Service.loadServiceById(oldServices.get(i));
            if(oldShifts.get(i) != 0) a.shift = Shift.loadShiftById(oldShifts.get(i));
            // a.reservedBy = null;
            if(oldReservedBys.get(i) != 0) a.reservedBy = UserManager.getOrganizerById(oldReservedBys.get(i));
        }

        for (Availability a: newAvailabilities) {
            loadedAvailabilities.put(a.id, a);
        }
